...
public class PlatformerMotor2D : MonoBehaviour
{
    ...
    private void ApplyMovement()
    {
        ...
        else if (Mathf.Abs(normalizedXMovement) > 0)
        {
            if (IsGrounded())
            {
				...
				cam = gameObject.transform.Find("Camera").gameObject;
				GameObject ninjaobject = GameObject.Find ("Ninja");
				LeNinja ninjascript = ninjaobject.GetComponent<LeNinja> ();
				if (ninjascript.FpsCamBool == true) {
					...
					if (cam.transform.eulerAngles.y >= 181 && cam.transform.eulerAngles.y <= 360) {
						Debug.Log ("Retourné");
						groundSpeed = -15;
				...
				if (ninjascript.FpsCamBool == false) {
            ...
            else if (changeDirectionInAir)
            {
                ...
                if (timeToAirSpeed > 0)
                {
                    if (_velocity.x > 0 &&
                        normalizedXMovement > 0 &&
						_velocity.x > normalizedXMovement * (groundSpeed*0.9f) ||
                        _velocity.x < 0 &&
                        normalizedXMovement < 0 &&
						_velocity.x < normalizedXMovement * (groundSpeed*0.9f))
                    {
                        speed = Decelerate(
                            _velocity.x,
							((groundSpeed*0.9f) * (groundSpeed*0.9f)) / (2 * airStopDistance),
							normalizedXMovement * (groundSpeed*0.9f));
                    }
                    else
                    {
                        speed = Accelerate(
                            _velocity.x,
							normalizedXMovement * ((groundSpeed*0.9f) / timeToAirSpeed),
							normalizedXMovement * (groundSpeed*0.9f));
                    }
                }
                else
                {
					speed = normalizedXMovement * (groundSpeed*0.9f);
                }
        ...
        else if (_velocity.x != 0)
        {
            ...
            else
            {
                ...
                {
					speed = Decelerate(_velocity.x, ((groundSpeed*0.75f) * (groundSpeed*0.75f)) / (2 * airStopDistance), 0);
                }
                else
                {
                    speed = 0;
    ...
    private float GetDashSpeed()
    {
		...
		if (cam.transform.eulerAngles.y >= 0 && cam.transform.eulerAngles.y <= 180) {
			...
			speed = ( _dashDerivativeFunction(0, dashDistance, normalizedTime) / dashDuration);
